...
async function run() {
	...
	document.getElementById('test-data-size-slider').addEventListener('input', updateSliderValue);
}

document.addEventListener('DOMContentLoaded', run);
let model;
async function startTraining(data) {
    const batchSize = parseInt(document.getElementById('batch-size-value').textContent);
    const trainDataSize = parseInt(document.getElementById('train-data-size-value').textContent);
    const testDataSize = parseInt(document.getElementById('test-data-size-value').textContent);

    model = getModel();
    tfvis.show.modelSummary({ name: 'Model Architecture', tab: 'Model' }, model);
    await train(model, data, batchSize, trainDataSize, testDataSize);
    await showAccuracy(model, data);
    await showConfusion(model, data);
	return model;
	}
...
async function predict() {
	if (!model) {
        ...
        infoDiv.innerHTML = "Model not trained yet.";
        return;
    }

    predictData = new MnistData();
    await predictData.load();

    // Get a random image from the dataset for prediction
    const testDataSize = 1; // Number of images to predict
    const [testXs, testYs] = tf.tidy(() => {
        const testData = predictData.nextTestBatch(testDataSize);
        return [
